  ...
  const generateReport = () => {
    ...
    for (const key in groupedInventory) {
      if (groupedInventory.hasOwnProperty(key)) {
        ...
        const receivedStock = items.reduce((total, item) => total + parseInt(item.received_stock), 0);
        const itemTotalCost = unitPrice * receivedStock;
        totalCost += itemTotalCost;
        content += `
          <tr>
            <td style="border: 1px solid #ccc; padding: 8px;">${materialType === 'Other' ? `${materialType} (${items[0].name})` : materialType} (${receivedStock} x Rs.${unitPrice.toFixed(2)})</td>
            <td style="border: 1px solid #ccc; padding: 8px;">${color}</td>
            <td style="border: 1px solid #ccc; padding: 8px;">${itemTotalCost.toFixed(2)}</td>
          </tr>`;
    ...
    content += `
        ...
        <p style="width: 60%; text-align: center; margin: 20px auto; font-weight: bold; background-color:#000000; color:#FFFFFF; padding:15px">Total Cost: Rs.${totalCost.toFixed(2)}</p>
  ...
  return (
    <div>
      <div style={{ marginTop: '80px', border: '1px solid #000000', marginLeft: '40px', marginRight: '40px' }}>
        <h1 style={{ textAlign: 'center', marginTop: '30px', marginBottom: '30px' }}>Inventory Report</h1>
        <button
