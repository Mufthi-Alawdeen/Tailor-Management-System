...
const { generateOrderID, generateTransactionID } = require('../utils/generator');
...
router.get('/buyproducts/suits', async (req, res) => {
    try {
        ...
        const buySuitProducts = await Product.find({ type: 'buy', category: 'suit' });
...
router.get('/buyproducts/shirts', async (req, res) => {
    try {
        // Fetch buy products of category 'suit' from the database
        const buySuitProducts = await Product.find({ type: 'buy', category: 'shirt' });
        res.json(buySuitProducts);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
router.get('/buyproducts/trousers', async (req, res) => {
    try {
        // Fetch buy products of category 'suit' from the database
        const buySuitProducts = await Product.find({ type: 'buy', category: 'trousers' });
        res.json(buySuitProducts);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
router.get('/buyproducts/accessories', async (req, res) => {
    try {
        ...
        const buyAccessoryProducts = await Product.find({ type: 'buy', category: { $in: ['bow', 'tie'] } });
...
router.post('/products/cart/add', async (req, res) => {
    const { productId, userId, fabric, color, type, measurements, quantity } = req.body;

    // Validation
    if (!productId || !userId || !fabric || !color || !type || !measurements || !quantity) {
    ...
    try {
        ...
        if (!inventoryItem) {
            return res.status(404).json({ message: 'No inventory item found matching the criteria' });
        }
        ...
        res.status(201).json({ message: 'Customized item added to cart successfully' });
    } catch (error) {
...
router.post('/createorder', async (req, res) => {
    try {
        ...
        const cartResponse = await axios.get(`http://localhost:8075/order/carts/${userObjectId}`);
        ...
        res.status(201).json({ transaction: newTransaction, orders: orders, message: 'Orders created successfully' });
    } catch (error) {
        // Handle errors
...
const calculatePickupDate = (carts) => {
    ...
    const trousersItems = carts.filter(item => item.product.category === 'trousers');
    ...
    const pickupDate = new Date(currentDate.getTime() + totalDays * 24 * 60 * 60 * 1000);
